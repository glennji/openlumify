...
import org.openlumify.core.model.user.UserPropertyAuthorizationRepository;
...
import org.openlumify.web.clientapi.model.ClientApiPublishItem.Action;
import org.openlumify.web.clientapi.model.ClientApiWorkspaceDiff.EdgeItem;
import org.openlumify.web.clientapi.model.ClientApiWorkspaceDiff.PropertyItem;
import org.openlumify.web.clientapi.model.ClientApiWorkspaceDiff.VertexItem;
...
public class VertexiumWorkspaceSandboxingTest extends OpenLumifyInMemoryTestBase {
    ...
    public void before() throws Exception {
        ...
        user1 = getUserRepository().findOrAddUser("user1", "user1", "user1@openlumify.com", "password");
        user2 = getUserRepository().findOrAddUser("user2", "user2", "user2@openlumify.com", "password");
        ...
        propertyDefinition = new OntologyPropertyDefinition(
                Collections.singletonList(thing),
                OpenLumifyProperties.TITLE.getPropertyName(),
                "Title",
                PropertyType.STRING
        );
        propertyDefinition.setTextIndexHints(Collections.singleton(TextIndexHint.EXACT_MATCH));
        ...
        propertyDefinition = new OntologyPropertyDefinition(
                Collections.singletonList(thing),
                OpenLumifyProperties.MODIFIED_BY.getPropertyName(),
                "Visibility JSON",
                PropertyType.DATE
        );
        getOntologyRepository().getOrCreateProperty(propertyDefinition, systemUser, PUBLIC);
    ...
    public void publishNewVertexSucceeds() {
        ...
        assertEquals(
                VERTEX_CONCEPT_TYPE,
                vertex.getProperty(OpenLumifyProperties.CONCEPT_TYPE.getPropertyName(), initialVisibility).getValue()
        );
        assertEquals(VERTEX_TITLE, OpenLumifyProperties.TITLE.getOnlyPropertyValue(vertex));
        assertEquals(
                VisibilityJson.removeFromAllWorkspace(initialVisibilityJson),
                OpenLumifyProperties.VISIBILITY_JSON.getPropertyValue(vertex)
        );
        assertNull(vertex.getProperty(OpenLumifyProperties.CONCEPT_TYPE.getPropertyName(), initialWorkspaceViz));
        assertNull(vertex.getProperty(OpenLumifyProperties.TITLE.getPropertyName(), initialWorkspaceViz));
        assertNull(vertex.getProperty(OpenLumifyProperties.VISIBILITY_JSON.getPropertyName(), initialWorkspaceViz));
    }
    ...
    public void undoDeleteVertexSucceeds() {
        ...
        assertEquals(VERTEX_CONCEPT_TYPE, OpenLumifyProperties.CONCEPT_TYPE.getPropertyValue(vertex));
        assertEquals(VERTEX_TITLE, OpenLumifyProperties.TITLE.getPropertyValue(vertex, ""));
        assertEquals(
    ...
    public void recreatedVertexWithSameIdAfterUndoShouldNotHaveOldProperties() {
        ...
        OpenLumifyProperties.VISIBILITY_JSON_METADATA.setMetadata(propertyMetadata, initialVisibilityJson,
                getVisibilityTranslator().getDefaultVisibility()
        );
    ...
    public void publishCommentOnPublishedVertexSucceeds() {
        ...
        publishAllWorkspaceDiffs();
        OpenLumifyProperties.COMMENT.addPropertyValue(v, "", "This is a test comment", initialMetadata, initialVisibility, workspaceAuthorizations);
        List<ClientApiWorkspaceDiff.Item> diffs = getDiffsFromWorkspace();
        assertEquals(1, diffs.size());
        publishAllWorkspaceDiffs();
        diffs = getDiffsFromWorkspace();
    ...
    private Edge newEdgeOnWorkspace() {
        ...
        OpenLumifyProperties.VISIBILITY_JSON.setProperty(edgeBuilder, initialVisibilityJson, new Visibility(""));
        OpenLumifyProperties.MODIFIED_BY.setProperty(edgeBuilder, "testUser", initialWorkspaceViz);
    ...
    private Vertex newVertexOnWorkspace() {
        ...
        OpenLumifyProperties.VISIBILITY_JSON.setProperty(vertexBuilder, initialVisibilityJson, new Visibility(""));
        ...
        OpenLumifyProperties.TITLE.addPropertyValue(
                vertexBuilder,
                "",
                VERTEX_TITLE,
                propertyMetadata,
    ...
    private static void assertVisibilityOnProperty(Visibility expectedVisibility, Property property) {
        ...
        VisibilityJson actualVisibilityJson = OpenLumifyProperties.VISIBILITY_JSON_METADATA.getMetadataValue(metadata);
    ...
    private static String extractVisibilitySource(String visibilityString) {
        return stripParenthesis(visibilityString)
                .replaceAll("[\\&\\|]", "")
                .replace(OpenLumifyVisibility.SUPER_USER_VISIBILITY_STRING, "");
    }
}
