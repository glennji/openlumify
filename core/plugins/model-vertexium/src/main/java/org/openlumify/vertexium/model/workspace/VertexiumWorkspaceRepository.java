...
public class VertexiumWorkspaceRepository extends WorkspaceRepository {
    private static final OpenLumifyLogger LOGGER = OpenLumifyLoggerFactory.getLogger(VertexiumWorkspaceRepository.class);
    ...
    public Iterable<Workspace> findAll(User user) {
        if (!user.equals(userRepository.getSystemUser())) {
            throw new OpenLumifyAccessDeniedException("Only system user can access all workspaces", user, null);
        }
        Authorizations authorizations = getAuthorizationRepository().getGraphAuthorizations(
                user,
                VISIBILITY_STRING,
        ...
        QueryResultsIterable<Vertex> workspaceVertices = getGraph().query(authorizations)
                .has(OpenLumifyProperties.CONCEPT_TYPE.getPropertyName(), Compare.EQUAL, WORKSPACE_CONCEPT_IRI)
                .vertices();
    ...
    private String getProductPreviewDataMd5(Vertex productVertex, User user) {
        ...
        if (previewDataUrlProperty != null) {
            Metadata.Entry entry = previewDataUrlProperty.getMetadata().getEntry("http://openlumify.org/product#previewImageMD5");
            if (entry != null) {
        ...
        try (GraphUpdateContext ctx = graphRepository.beginGraphUpdate(Priority.NORMAL, user, authorizations)) {
            productVertex = ctx.getOrCreateVertexAndUpdate(productId, visibility, elCtx -> {
                if (preview == null) {
                    ...
                    metadata.add("http://openlumify.org/product#previewImageMD5", preview.getMD5(), visibility);
    ...
    public Product addOrUpdateProduct(String workspaceId, String productId, String title, String kind, JSONObject params, User user) {
        ...
        try (GraphUpdateContext ctx = graphRepository.beginGraphUpdate(Priority.NORMAL, user, authorizations)) {
            productVertex = ctx.getOrCreateVertexAndUpdate(productId, visibility, elemCtx -> {
                ...
                if (kind != null) {
                    if (getWorkProductServiceByKind(kind) == null) {
                        throw new OpenLumifyException("invalid kind: " + kind);
                    }
            ...
            throw new OpenLumifyException("Could not addOrUpdateProduct(workspaceId: " + workspaceId + ", productId: " + productId + ")", e);
        }
    ...
    private void updateProductData(ElementUpdateContext<Vertex> elemCtx, StringOpenLumifyProperty property, JSONObject data) {
        if (data == null) {
            return;
        }
        Visibility visibility = VISIBILITY.getVisibility();
    ...
    public WorkProductAncillaryResponse addOrUpdateProductAncillaryVertex(
        ...
        Vertex productVertex = getGraph().getVertex(productId, authorizations);
        if (productVertex == null) {
            throw new OpenLumifyException("Unable to update vertex, productId not found: " + productId);
        }

        String kind = WorkspaceProperties.PRODUCT_KIND.getPropertyValue(productVertex);
        WorkProductService service = getWorkProductServiceByKind(kind);
        if (!(service instanceof WorkProductServiceHasElements)) {
            throw new OpenLumifyException("Service doesn't support entities");
        }
        ...
        try (GraphUpdateContext ctx = graphRepository.beginGraphUpdate(Priority.NORMAL, user, authorizations)) {
            ...
            throw new OpenLumifyException("Unable to add or update vertex", e);
        }
    ...
    public void deleteUserFromWorkspace(final Workspace workspace, final String userId, final User user) {
        ...
        lockRepository.lock(getLockName(workspace), () -> {
            ...
            if (userVertex == null) {
                throw new OpenLumifyResourceNotFoundException("Could not find user: " + userId, userId);
            }
            Vertex workspaceVertex = getVertexFromWorkspace(workspace, true, authorizations);
