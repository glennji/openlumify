...
public class InMemoryOntologyRepository extends OntologyRepositoryBase {
    private static final OpenLumifyLogger LOGGER = OpenLumifyLoggerFactory.getLogger(InMemoryOntologyRepository.class);
    ...
    protected void setIconProperty(
        ...
        if (glyphIconFileName == null) {
            ...
            try {
                ...
                throw new OpenLumifyException("Failed to set glyph icon to " + iconFile, ex);
            }
        }
    }
    ...
    protected List<OWLOntology> loadOntologyFiles(
        ...
        for (OwlData owlData : fileCacheCopy) {
            IRI openlumifyBaseOntologyIRI = IRI.create(owlData.iri);
            if (excludedIRI != null && excludedIRI.equals(openlumifyBaseOntologyIRI)) {
                continue;
            }
            try (InputStream openlumifyBaseOntologyIn = new ByteArrayInputStream(owlData.data)) {
                ...
                OWLOntologyDocumentSource openlumifyBaseOntologySource = new ReaderDocumentSource(
                        openlumifyBaseOntologyReader,
                        openlumifyBaseOntologyIRI
                );
    ...
    protected Concept internalGetOrCreateConcept(Concept parent, String conceptIRI, String displayName, String glyphIconHref, String color, File inDir, boolean deleteChangeableProperties, User user, String workspaceId) {
        ...
        if (conceptIRI.equals(OntologyRepository.ENTITY_CONCEPT_IRI)) {
            concept.setProperty(OntologyProperties.TITLE_FORMULA.getPropertyName(), "prop('http://openlumify.org#title') || ''", user, null);

            // TODO: change to ontology && ontology.displayName
            concept.setProperty(OntologyProperties.SUBTITLE_FORMULA.getPropertyName(), "prop('http://openlumify.org#source') || ''", user, null);
