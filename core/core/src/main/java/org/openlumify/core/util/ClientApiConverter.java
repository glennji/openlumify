...
import org.openlumify.core.ingest.video.VideoPropertyHelper;
...
public class ClientApiConverter extends org.openlumify.web.clientapi.util.ClientApiConverter {
    private static final OpenLumifyLogger LOGGER = OpenLumifyLoggerFactory.getLogger(ClientApiConverter.class);
    ...
    private static void populateClientApiElement(
        ...
        if (clientApiElement instanceof ClientApiVertex) {
            ...
            String conceptType = OpenLumifyProperties.CONCEPT_TYPE.getPropertyValue(element, null);
    ...
    public static ClientApiProperty toClientApiProperty(Property property) {
        ...
        } else if (OpenLumifyProperties.VISIBILITY_JSON.isSameName(property)) {
    ...
    private static void addVideoFramePropertyToResults(
        ...
        ClientApiProperty clientApiProperty = findProperty(
                clientApiProperties,
                MediaOpenLumifyProperties.VIDEO_TRANSCRIPT.getPropertyName(),
                propertyKey
        );
        if (clientApiProperty == null) {
            clientApiProperty = new ClientApiProperty();
            clientApiProperty.setKey(propertyKey);
            clientApiProperty.setName(MediaOpenLumifyProperties.VIDEO_TRANSCRIPT.getPropertyName());
            clientApiProperty.setSandboxStatus(sandboxStatus);
    ...
    public static ClientApiHistoricalPropertyResults calculateHistoricalPropertyDeltas(
        ...
        for (HistoricalPropertyValue hpv : sortedHistoricalValues) {
            ...
            if (cached == null) { // Add
                if (hpv.getPropertyName().equals(OpenLumifyProperties.CONCEPT_TYPE.getPropertyName())) {
                    conceptTypeHpv = hpv;
                    HistoricalPropertyValue modifiedByHpv = cachedValues.get(OpenLumifyProperties.MODIFIED_BY.getPropertyName());

                    int j = i;
                    while (modifiedByHpv == null && j < sortedHistoricalValues.size()) {
                        HistoricalPropertyValue v = sortedHistoricalValues.get(j);
                        if (v.getPropertyName().equals(OpenLumifyProperties.MODIFIED_BY.getPropertyName())) {
                            modifiedByHpv = v;
                        }
                        j++;
                    }
    ...
    private static ClientApiHistoricalPropertyResults.Event generateGenericEvent(HistoricalPropertyValue hpv) {
        ...
        Metadata.Entry modifiedByEntry = (hpv.getMetadata() != null) ? hpv.getMetadata().getEntry(OpenLumifyProperties.MODIFIED_BY.getPropertyName()) : null;
    ...
    private static String readStreamingPropertyValueStringForHistory(StreamingPropertyValue spv) {
        try (InputStream in = spv.getInputStream()) {
            ...
            throw new OpenLumifyException("Could not read StreamingPropertyValue", ex);
        }
    }
    ...
    public static String removeWorkspaceVisibility(String visibility) {
        return Arrays.stream(visibility.split("\\|"))
                .map(s -> s.replaceAll("&?\\(WORKSPACE_.*?\\)", "").replace("openlumify", ""))
