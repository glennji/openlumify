...
import org.openlumify.core.model.lock.LeaderListener;
import org.openlumify.core.model.lock.LockRepository;

import java.util.Date;

public abstract class PeriodicBackgroundService {
    private static final OpenLumifyLogger LOGGER = OpenLumifyLoggerFactory.getLogger(PeriodicBackgroundService.class);
        ...
        Thread t = new Thread(() -> {
            ...
            while (true) {
                try {
                    ...
                    throw new OpenLumifyException("Failed to sleep", e);
                }
                runPeriodically();
            }
        });
    ...
    protected String getThreadName() {
        return "openlumify-periodic-" + this.getClass().getSimpleName();
    }
