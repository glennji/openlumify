...
import org.openlumify.core.util.StoppableRunnable;
...
public class LongRunningProcessRunner extends WorkerBase<LongRunningProcessWorkerItem> {
    private static final OpenLumifyLogger LOGGER = OpenLumifyLoggerFactory.getLogger(LongRunningProcessRunner.class);
    ...
    private void prepareWorkers(Map map) {
        ...
        for (LongRunningProcessWorker worker : InjectHelper.getInjectedServices(LongRunningProcessWorker.class, configuration)) {
            try {
                ...
                throw new OpenLumifyException("Could not prepare graph property worker " + worker.getClass().getName(), ex);
            }
            workers.add(worker);
        }
    }
