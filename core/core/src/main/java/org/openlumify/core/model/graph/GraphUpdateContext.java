 ...
 *     OpenLumifyProperties.FILE_NAME.updateProperty(updateContext, "key", fileName, metadata, visibility);
 *   });
 * }
 * }
 * </pre>
...
public abstract class GraphUpdateContext implements AutoCloseable {
    ...
    private void pushOutstandingUpdateFutures() {
        outstandingFutures.forEach(f -> {
            try {
                workQueueRepository.pushGraphOpenLumifyPropertyQueue(f.get(), f.getElementUpdateContext().getProperties(), priority);
            } catch (Exception ex) {
                throw new OpenLumifyException("Could not push on queue", ex);
            }
        });
    }
    ...
    public <T extends Element> UpdateFuture<T> update(
        ...
        try {
            ...
            throw new OpenLumifyException("Could not update element", ex);
        }
    ...
    public class UpdateFuture<T extends Element> implements Future<T> {
        ...
        public boolean cancel(boolean mayInterruptIfRunning) {
            throw new OpenLumifyException("Not supported");
        }
