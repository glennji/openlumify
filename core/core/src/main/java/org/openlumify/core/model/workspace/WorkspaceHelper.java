...
public class WorkspaceHelper {
    private static final OpenLumifyLogger LOGGER = OpenLumifyLoggerFactory.getLogger(WorkspaceHelper.class);
    //TODO fix this key when there's a migration capability
    private static final String DETECTED_OBJECT_MULTI_VALUE_KEY_PREFIX = "org.openlumify.web.routes.vertex.ResolveDetectedObject";
    ...
    public String getWorkspaceIdOrNullIfPublish(
        ...
        if (shouldPublish) {
            ...
            throw new OpenLumifyException("workspaceId parameter required");
        }
        return workspaceId;
    }
    ...
    public void unresolveTerm(Vertex termMention, Authorizations authorizations) {
        ...
        String resolveEdgeId = OpenLumifyProperties.TERM_MENTION_RESOLVED_EDGE_ID.getPropertyValue(termMention, null);
    ...
    public void deleteEdge(
        ...
        if (isPublicEdge) {
            ...
            if (edge.getLabel().equals(entityHasImageIri)) {
                Property entityHasImage = outVertex.getProperty(OpenLumifyProperties.ENTITY_IMAGE_VERTEX_ID.getPropertyName());
    ...
    public void deleteProperties(
        ...
        if (workspaceId != null) {
            if (e instanceof Edge) {
                ...
                throw new OpenLumifyException("element is not an edge or vertex: " + e);
            }
        }
    ...
    public void deleteVertex(
        ...
        for (Edge edge : vertex.getEdges(Direction.BOTH, artifactContainsImageOfEntityIri, authorizations)) {
            for (Property rowKeyProperty : vertex.getProperties(OpenLumifyProperties.ROW_KEY.getPropertyName())) {
                String multiValueKey = rowKeyProperty.getValue().toString();
                if (edge.getVertexId(Direction.IN).equals(vertex.getId())) {
                    Vertex outVertex = edge.getVertex(Direction.OUT, authorizations);
                    // remove property
                    OpenLumifyProperties.DETECTED_OBJECT.removeProperty(outVertex, multiValueKey, authorizations);
                    graph.softDeleteEdge(edge, authorizations);
                    workQueueRepository.pushEdgeDeletion(edge, beforeActionTimestamp, Priority.HIGH);
                    workQueueRepository.pushGraphPropertyQueue(
                            outVertex,
                            multiValueKey,
                            OpenLumifyProperties.DETECTED_OBJECT.getPropertyName(),
                            workspaceId,
    ...
    private void unresolveTermMentionsForProperty(Vertex vertex, Property property, Authorizations authorizations) {
        for (Vertex termMention : termMentionRepository.findResolvedTo(vertex.getId(), authorizations)) {
            String key = OpenLumifyProperties.TERM_MENTION_REF_PROPERTY_KEY.getPropertyValue(termMention);
            String name = OpenLumifyProperties.TERM_MENTION_REF_PROPERTY_NAME.getPropertyValue(termMention);
            String visibility = OpenLumifyProperties.TERM_MENTION_REF_PROPERTY_VISIBILITY.getPropertyValue(termMention);
    ...
    private void unresolveDetectedObjects(
        ...
        for (ArtifactDetectedObject artifactDetectedObject : OpenLumifyProperties.DETECTED_OBJECT.getPropertyValues(
                outVertex)) {
    ...
    private void unresolveDetectedObject(
        ...
        if (vertexSandboxStatus == SandboxStatus.PUBLIC) {
            ...
            visibilityJson = OpenLumifyProperties.VISIBILITY_JSON.getPropertyValue(inVertex);
            visibilityJson = VisibilityJson.removeFromWorkspace(visibilityJson, workspaceId);
        }
        OpenLumifyProperties.DETECTED_OBJECT.removeProperty(outVertex, multiValueKey, authorizations);
