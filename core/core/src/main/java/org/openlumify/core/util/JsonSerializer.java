...
public class JsonSerializer {
    ...
    public static JSONArray toJsonProperties(Iterable<Property> properties, String workspaceId) {
        ...
        for (int i = 0; i < propertiesList.size(); i++) {
            ...
            if ((videoFrameInfo = VideoPropertyHelper.getVideoFrameInfoFromProperty(property)) != null) {
                String textDescription = OpenLumifyProperties.TEXT_DESCRIPTION_METADATA.getMetadataValueOrDefault(property.getMetadata(), null);
    ...
    private static void addVideoFramePropertyToResults(JSONArray resultsJson, String propertyKey, String textDescription, String sandboxStatus) {
        JSONObject json = findProperty(resultsJson, MediaOpenLumifyProperties.VIDEO_TRANSCRIPT.getPropertyName(), propertyKey);
        if (json == null) {
            json = new JSONObject();
            json.put("key", propertyKey);
            json.put("name", MediaOpenLumifyProperties.VIDEO_TRANSCRIPT.getPropertyName());
            json.put("sandboxStatus", sandboxStatus);
            json.put(OpenLumifyProperties.TEXT_DESCRIPTION_METADATA.getMetadataKey(), textDescription);
