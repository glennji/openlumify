...
import org.openlumify.core.model.user.cli.args.*;
...
public class UserAdmin extends CommandLineTool {
    private static final OpenLumifyLogger LOGGER = OpenLumifyLoggerFactory.getLogger(UserAdmin.class, "cli-userAdmin");
    ...
    protected JCommander parseArguments(String[] args) {
        ...
        if (userAdminAction == null) {
            throw new OpenLumifyException("Could not parse UserAdminAction");
        }
    ...
    private Args getArgumentsObject() {
        ...
        throw new OpenLumifyException("Unhandled userAdminAction: " + userAdminAction);
    }
    ...
    private int exportPasswords(ExportPasswordsArgs args) {
        ...
        if (!sortedUsers.isEmpty()) {
            ...
            for (User user : sortedUsers) {
                String passwordSalt = Base64.getEncoder().encodeToString((byte[]) user.getProperty(UserOpenLumifyProperties.PASSWORD_SALT.getPropertyName()));
                String passwordHash = Base64.getEncoder().encodeToString((byte[]) user.getProperty(UserOpenLumifyProperties.PASSWORD_HASH.getPropertyName()));
                System.out.printf(
                        format,
                        user.getUsername(),
