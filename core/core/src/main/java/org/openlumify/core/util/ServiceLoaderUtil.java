...
public class ServiceLoaderUtil {
    private static final OpenLumifyLogger LOGGER = OpenLumifyLoggerFactory.getLogger(ServiceLoaderUtil.class);
        ...
        return Iterables.transform(classes, new Function<Class<? extends T>, T>() {
            ...
            public T apply(Class<? extends T> serviceClass) {
                try {
                    ...
                    throw new OpenLumifyException(errorMessage, ex);
                }
            }
        });
    }
    ...
    public static <T> Iterable<Class<? extends T>> loadClasses(Class<T> clazz, Configuration configuration) {
        ...
        try {
            ...
            if (!serviceFiles.hasMoreElements()) {
                ...
                for (URL serviceFile : serviceFilesSet) {
                    ...
                    for (String className : fileClassNames) {
                        ...
                        OpenLumifyPlugin openlumifyPlugin = loadedClass.getAnnotation(OpenLumifyPlugin.class);
                        if (openlumifyPlugin != null) {
                            if (configuration.getBoolean(CONFIG_DISABLE_PREFIX + className, openlumifyPlugin.disabledByDefault())) {
            ...
            throw new OpenLumifyException("Could not load services for class: " + clazz.getName(), e);
        }
    }
    ...
    private static <T> Class<? extends T> loadClass(URL config, String className) {
        try {
            ...
            throw new OpenLumifyException(errorMessage, t);
        }
    }
}
