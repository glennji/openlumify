...
public class Configuration {
    private static final OpenLumifyLogger LOGGER = OpenLumifyLoggerFactory.getLogger(Configuration.class);
    ...
    public static final String DEFAULT_LOCK_REPOSITORY_PATH_PREFIX = "/openlumify/locks";
    ...
    public static final String DEFAULT_CACHE_REPOSITORY_PATH_PREFIX = "/openlumify/cache";
    ...
    public static final String DEFAULT_STATUS_ZK_PATH = "/openlumify/status";
    ...
    public static final String SYSTEM_PROPERTY_PREFIX = "openlumify.";
    ...
    public Configuration(final ConfigurationLoader configurationLoader, final Map<?, ?> config) {
        ...
        this.openlumifyResourceBundleManager = new OpenLumifyResourceBundleManager(this);
    ...
    public <T> Class<? extends T> getClass(String propertyKey, Class<? extends T> defaultClass) {
        ...
        if (className == null) {
            throw new OpenLumifyException("Could not find required property " + propertyKey);
        }
        ...
        try {
            ...
            throw new OpenLumifyException("Could not load class " + className + " for property " + propertyKey, e);
        }
    }
        ...
        for (Method m : o.getClass().getMethods()) {
            ...
            if (validatorAnnotation != null) {
                if (m.getParameterTypes().length != 0) {
                    throw new OpenLumifyException("Invalid validator method " + o.getClass().getName() + "." + m.getName() + "(). Expected 0 arguments. Found " + m.getParameterTypes().length + " arguments");
                }
                if (m.getReturnType() != Boolean.TYPE) {
                    throw new OpenLumifyException("Invalid validator method " + o.getClass().getName() + "." + m.getName() + "(). Expected Boolean return type. Found " + m.getReturnType());
                }
        ...
        for (Method postConfigurationValidatorMethod : validatorMap.keySet()) {
            try {
                if (!(Boolean) postConfigurationValidatorMethod.invoke(o)) {
                    ...
                    throw new OpenLumifyException(o.getClass().getName() + "." + postConfigurationValidatorMethod.getName() + description + " returned false");
                }
            } catch (InvocationTargetException e) {
                throw new OpenLumifyException("InvocationTargetException invoking validator " + o.getClass().getName() + "." + postConfigurationValidatorMethod.getName(), e);
            } catch (IllegalAccessException e) {
                throw new OpenLumifyException("IllegalAccessException invoking validator " + o.getClass().getName() + "." + postConfigurationValidatorMethod.getName(), e);
            }
        }
    ...
    private static void setConfigurablesMethod(Object o, Method m, Map<String, String> config, ConvertUtilsBean convertUtilsBean) {
        ...
        if (m.getParameterTypes().length != 1) {
            throw new OpenLumifyException("Invalid method to be configurable. Expected 1 argument. Found " + m.getParameterTypes().length + " arguments");
        }
        ...
        setConfigurable(config, configurableAnnotation, propName, m.getParameterTypes()[0], false, convertUtilsBean, value -> {
            try {
                ...
                throw new OpenLumifyException("Could not set property " + m.getName() + " on " + o.getClass().getName());
            }
        });
    }
    ...
    private static void setConfigurablesField(Object o, Field f, Map<String, String> config, ConvertUtilsBean convertUtilsBean) {
        ...
        setConfigurable(config, configurableAnnotation, propName, f.getType(), true, convertUtilsBean, value -> {
            try {
                ...
                throw new OpenLumifyException("Could not set property " + f.getName() + " on " + o.getClass().getName());
            }
        });
    }
        ...
        if (config.containsKey(name)) {
            ...
            if (Configurable.DEFAULT_VALUE.equals(configurableAnnotation.defaultValue())) {
                ...
                if (configurableAnnotation.required()) {
                    throw new OpenLumifyException(String.format("Could not find property \"%s\" and no default value was specified.", name));
                } else {
                    return;
                }
            }
    ...
    public JSONObject toJSON(Locale locale, String workspaceId) {
        ...
        return toJSON(openlumifyResourceBundleManager.getBundle(locale), workspaceId);
    }
     ...
     * <p>
     * <pre><code>
     * repository.ontology.owl.dev.iri=http://openlumify.org/dev
     * repository.ontology.owl.dev.dir=examples/ontology-dev/
     *
     * repository.ontology.owl.csv.iri=http://openlumify.org/csv
    ...
    public <T> Map<String, T> getMultiValueConfigurables(String prefix, Class<T> configurableType) {
        ...
        for (Map.Entry<String, Map<String, String>> entry : multiValues.entrySet()) {
            ...
            try {
                ...
                throw new OpenLumifyException("Could not create configurable: " + configurableType.getName(), e);
            }
     ...
     * <p/>
     * - dev
     * - iri: http://openlumify.org/dev
     * - dir: examples/ontology-dev/
     * - csv
     * - iri: http://openlumify.org/csv
