...
public abstract class ConfigurationLoader {
    ...
    public static Class getConfigurationLoaderByName(String configLoaderName) {
        ...
        try {
            ...
            throw new OpenLumifyException("Could not load class " + configLoaderName, e);
        }
        return configLoader;
    }
    ...
    private static ConfigurationLoader createConfigurationLoader(Class configLoaderClass, Map initParameters) {
        ...
        try {
            ...
            throw new OpenLumifyException("Could not load configuration class: " + configLoaderClass.getName(), e);
        }
    }
    ...
    protected void doConfigureLog4j() {
        ...
        } catch (OpenLumifyResourceNotFoundException e) {
            // OK, try classpath
        }
        if (log4jFile == null || !log4jFile.exists()) {
            ...
            try {
                if (log4jResource != null) {
                    ...
                    throw new OpenLumifyResourceNotFoundException("Could not find log4j2.xml on the classpath");
                }
            } catch (URISyntaxException e) {
                throw new OpenLumifyException("Unable to load default log42.xml", e);
            }
        } else {
            log4jLocation = log4jFile.getAbsolutePath();
            Configurator.initialize((String)null, null, log4jFile.toURI());
        }
        OpenLumifyLogger logger = OpenLumifyLoggerFactory.getLogger(OpenLumifyLoggerFactory.class);
