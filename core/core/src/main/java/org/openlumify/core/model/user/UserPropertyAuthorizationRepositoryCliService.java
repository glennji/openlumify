...
import org.openlumify.core.model.user.cli.AuthorizationRepositoryCliService;
import org.openlumify.core.model.user.cli.UserAdmin;
...
public class UserPropertyAuthorizationRepositoryCliService implements AuthorizationRepositoryCliService {
    ...
    public int run(UserAdmin userAdmin, String action, Args args, User authUser) {
        ...
        throw new OpenLumifyException("Unhandled cli action " + action);
    }
    ...
    private void validateCreateUserArguments(CreateUserArgs args) {
        ...
        if ((s == 1 && authsString == null) || s != 1) {
            throw new OpenLumifyException(this.getClass().getName() + " expects no parameters or '" + CLI_PARAMETER_AUTHORIZATIONS + "'");
        }
    }
