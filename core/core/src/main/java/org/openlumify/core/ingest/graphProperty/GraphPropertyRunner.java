...
import org.openlumify.core.model.WorkerBase;
...
public class GraphPropertyRunner extends WorkerBase<GraphPropertyWorkerItem> {
    private static final OpenLumifyLogger LOGGER = OpenLumifyLoggerFactory.getLogger(GraphPropertyRunner.class);
    ...
    public void prepareWorkers(GraphPropertyWorkerInitializer initializer) {
        if (prepareWorkersCalled) {
            throw new OpenLumifyException("prepareWorkers should be called only once");
        }
    ...
    private File copyToTempFile(InputStream in, GraphPropertyWorkData workData) throws IOException {
        String fileExt = null;
        String fileName = OpenLumifyProperties.FILE_NAME.getOnlyPropertyValue(workData.getElement());
        if (fileName != null) {
    ...
    private List<GraphPropertyThreadedWrapper> findInterestedWorkers(
        ...
        Set<String> graphPropertyWorkerWhiteList = IterableUtils.toSet(OpenLumifyProperties.GRAPH_PROPERTY_WORKER_WHITE_LIST.getPropertyValues(
                element));
        Set<String> graphPropertyWorkerBlackList = IterableUtils.toSet(OpenLumifyProperties.GRAPH_PROPERTY_WORKER_BLACK_LIST.getPropertyValues(
                element));
