...
public abstract class OntologyProperty {
    ...
    protected static Object parseGeoLocation(String valueStr) {
        try {
            ...
            throw new OpenLumifyException("Could not parse location: " + valueStr);
        }
    }
    ...
    public OpenLumifyProperty getOpenLumifyProperty() {
        switch (getDataType()) {
            case IMAGE:
            case BINARY:
                return new StreamingOpenLumifyProperty(getIri());
            case BOOLEAN:
                return new BooleanOpenLumifyProperty(getIri());
            case DATE:
                return new DateOpenLumifyProperty(getIri());
            case CURRENCY:
            case DOUBLE:
                return new DoubleOpenLumifyProperty(getIri());
            case GEO_LOCATION:
                return new GeoPointOpenLumifyProperty(getIri());
            case GEO_SHAPE:
                return new GeoShapeOpenLumifyProperty(getIri());
            case INTEGER:
                return new IntegerOpenLumifyProperty(getIri());
            case STRING:
            case DIRECTORY_ENTITY:
                return new StringOpenLumifyProperty(getIri());
            default:
                throw new OpenLumifyException("Could not get " + OpenLumifyProperty.class.getName() + " for data type " + getDataType());
        }
    }
