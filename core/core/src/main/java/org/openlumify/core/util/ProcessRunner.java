...
public class ProcessRunner {
    private static final OpenLumifyLogger LOGGER = OpenLumifyLoggerFactory.getLogger(ProcessRunner.class);
    ...
    public String executeToString(final String programName, final String[] programArgs) {
        try {
            ...
            throw new OpenLumifyException("Could not run " + programName + " " + Joiner.on(" ").join(programArgs), ex);
        }
    }
    ...
    public Process execute(
        ...
        if (proc.exitValue() != 0) {
            throw new OpenLumifyException("unexpected return code: " + proc.exitValue() + " for command " + arrayToString(arguments));
        }
        if (pipeException[0] != null) {
            throw new OpenLumifyException("pipe exception", pipeException[0]);
        }
