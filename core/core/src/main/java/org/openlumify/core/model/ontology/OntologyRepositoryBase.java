...
import org.openlumify.core.cache.CacheOptions;
...
import org.openlumify.core.model.longRunningProcess.LongRunningProcessProperties;
...
import org.openlumify.core.model.properties.types.OpenLumifyPropertyBase;
...
import org.openlumify.core.model.thumbnails.ThumbnailOntology;
...
import org.openlumify.core.ping.PingOntology;
import org.openlumify.core.user.SystemUser;
import org.openlumify.core.user.User;
import org.openlumify.core.util.ExecutorServiceUtil;
...
public abstract class OntologyRepositoryBase implements OntologyRepository {
    public static final String BASE_OWL_IRI = "http://openlumify.org";
    public static final String COMMENT_OWL_IRI = "http://openlumify.org/comment";
    ...
    private static final OpenLumifyLogger LOGGER = OpenLumifyLoggerFactory.getLogger(OntologyRepositoryBase.class);
    ...
    public void loadOntologies(Configuration config, Authorizations authorizations) throws Exception {
        lockRepository.lock("ontology", () -> {
            ...
            for (Map.Entry<String, Map<String, String>> owlGroup : config.getMultiValue(Configuration.ONTOLOGY_REPOSITORY_OWL).entrySet()) {
                ...
                if (iri == null) {
                    throw new OpenLumifyException("iri is required for group " + Configuration.ONTOLOGY_REPOSITORY_OWL + "." + owlGroup.getKey());
                }
                if (dir == null && file == null) {
                    throw new OpenLumifyException("dir or file is required for " + Configuration.ONTOLOGY_REPOSITORY_OWL + "." + owlGroup.getKey());
                }
                if (dir != null && file != null) {
                    throw new OpenLumifyException("you cannot specify both dir and file for " + Configuration.ONTOLOGY_REPOSITORY_OWL + "." + owlGroup.getKey());
                }

                if (dir != null) {
                    File owlFile = findOwlFile(new File(dir));
                    if (owlFile == null) {
                        throw new OpenLumifyResourceNotFoundException(
    ...
    public void importResourceOwl(Class baseClass, String fileName, String iri, Authorizations authorizations) {
        ...
        try {
            ...
            throw new OpenLumifyException("Could not import ontology file: " + fileName + " (iri: " + iri + ")", ex);
        } finally {
    ...
    private void addEntityGlyphIcon(Concept entityConcept, Authorizations authorizations) {
        ...
        try {
            ...
            throw new OpenLumifyException("invalid stream for glyph icon");
        }
    }
    ...
    public void importFile(File inFile, IRI documentIRI, Authorizations authorizations) throws Exception {
        ...
        if (!inFile.exists()) {
            throw new OpenLumifyException("File " + inFile + " does not exist");
        }
    ...
    protected Concept getParentConcept(
        ...
        if (superClasses.size() == 0) {
            ...
            if (parent == null) {
                throw new OpenLumifyException("Could not find or create parent: " + superClass);
            }
            return parent;
        } else {
            throw new OpenLumifyException("Unhandled multiple super classes. Found " + superClasses.size() + ", expected 0 or 1.");
        }
    }
    ...
    protected void importDataProperty(OWLOntology o, OWLDataProperty dataTypeProperty, Authorizations authorizations) {
        ...
        try {
            ...
            throw new OpenLumifyException("Failed to load data property: " + propertyIRI, ex);
        }
    }
    ...
    private Relationship getParentObjectProperty(
        ...
        if (superProperties.size() == 0) {
            ...
            if (parent == null) {
                throw new OpenLumifyException("Could not find or create parent: " + superProperty);
            }
            return parent;
        } else {
            throw new OpenLumifyException("Unhandled multiple super properties. Found " + superProperties.size() + ", expected 0 or 1.");
        }
    }
    ...
    public void writePackage(File file, IRI documentIRI, Authorizations authorizations) throws Exception {
        if (!file.exists()) {
            throw new OpenLumifyException("OWL file does not exist: " + file.getAbsolutePath());
        }
        if (!file.isFile()) {
            throw new OpenLumifyException("OWL file is not a file: " + file.getAbsolutePath());
        }
        ZipFile zipped = new ZipFile(file);
        if (zipped.isValidZipFile()) {
            File tempDir = Files.createTempDir();
    ...
    public OntologyProperty getRequiredPropertyByIRI(String propertyIRI, String workspaceId) {
        ...
        if (property == null) {
            throw new OpenLumifyException("Could not find property by IRI: " + propertyIRI);
        }
        return property;
    }
    ...
    public Concept getConceptByIntent(String intent, String workspaceId) {
        ...
        if (conceptIri != null) {
            Concept concept = getConceptByIRI(conceptIri, workspaceId);
            if (concept == null) {
                throw new OpenLumifyException("Could not find concept by configuration key: " + configurationKey);
            }
            return concept;
        }
        ...
        throw new OpenLumifyException("Found multiple concepts for intent: " + intent + " (" + iris + ")");
    }
    ...
    public Concept getRequiredConceptByIntent(String intent, String workspaceId) {
        Concept concept = getConceptByIntent(intent, workspaceId);
        if (concept == null) {
            throw new OpenLumifyException("Could not find concept by intent: " + intent);
        }
        return concept;
    }
    ...
    public Concept getRequiredConceptByIRI(String iri, String workspaceId) {
        ...
        if (concept == null) {
            throw new OpenLumifyException("Could not find concept by IRI: " + iri);
        }
        return concept;
    }
    ...
    public Relationship getRelationshipByIntent(String intent, String workspaceId) {
        ...
        if (relationshipIri != null) {
            ...
            if (relationship == null) {
                throw new OpenLumifyException("Could not find relationship by configuration key: " + configurationKey);
            }
            return relationship;
        }
        ...
        throw new OpenLumifyException("Found multiple relationships for intent: " + intent + " (" + iris + ")");
    }
    ...
    public Relationship getRequiredRelationshipByIntent(String intent, String workspaceId) {
        Relationship relationship = getRelationshipByIntent(intent, workspaceId);
        if (relationship == null) {
            throw new OpenLumifyException("Could not find relationship by intent: " + intent);
        }
        return relationship;
    }
    ...
    public OntologyProperty getPropertyByIntent(String intent, String workspaceId) {
        ...
        if (propertyIri != null) {
            OntologyProperty property = getPropertyByIRI(propertyIri, workspaceId);
            if (property == null) {
                throw new OpenLumifyException("Could not find property by configuration key: " + configurationKey);
            }
            return property;
        }
        ...
        throw new OpenLumifyException("Found multiple properties for intent: " + intent + " (" + iris + ")");
    }
    ...
    public OntologyProperty getRequiredPropertyByIntent(String intent, String workspaceId) {
        ...
        if (property == null) {
            throw new OpenLumifyException("Could not find property by intent: " + intent);
        }
        return property;
    }
    ...
    @Override
    public <T extends OpenLumifyProperty> T getOpenLumifyPropertyByIntent(String intent, Class<T> openlumifyPropertyType, String workspaceId) {
        ...
        try {
            Constructor<T> constructor = openlumifyPropertyType.getConstructor(String.class);
            return constructor.newInstance(propertyIri);
        } catch (Exception ex) {
            throw new OpenLumifyException("Could not create property for intent: " + intent + " (propertyIri: " + propertyIri + ")");
        }
    }

    @Override
    public <T extends OpenLumifyProperty> T getRequiredOpenLumifyPropertyByIntent(String intent, Class<T> openlumifyPropertyType, String workspaceId) {
        T result = getOpenLumifyPropertyByIntent(intent, openlumifyPropertyType, workspaceId);
        if (result == null) {
            throw new OpenLumifyException("Could not find property by intent: " + intent);
        }
        return result;
    ...
    protected void defineRequiredProperties(Graph graph) {
        definePropertyOnGraph(graph, OpenLumifyProperties.CONCEPT_TYPE, String.class, EnumSet.of(TextIndexHint.EXACT_MATCH));
        definePropertyOnGraph(graph, OpenLumifyProperties.MODIFIED_BY, String.class, EnumSet.of(TextIndexHint.EXACT_MATCH));
        definePropertyOnGraph(graph, OpenLumifyProperties.MODIFIED_DATE, Date.class, TextIndexHint.NONE);
        definePropertyOnGraph(graph, OpenLumifyProperties.VISIBILITY_JSON, String.class, TextIndexHint.NONE);
    ...
    protected void definePropertyOnGraph(Graph graph, OpenLumifyPropertyBase<?, ?> property, Class dataType, Set<TextIndexHint> textIndexHint) {
                ...
                if (concept.getSandboxStatus().equals(SandboxStatus.PRIVATE)) {
                    for (Relationship relationship : getRelationships(workspaceId)) {
                        if (relationship.getDomainConceptIRIs().contains(conceptTypeIri) ||
                            ...
                            throw new OpenLumifyException("Unable to delete concept that is used in domain/range of relationship");
                        }
                    }
                    ...
                    if (results == 0) {
                        ...
                        throw new OpenLumifyException("Unable to delete concept that have vertices assigned to it");
                    }
                } else {
                    throw new OpenLumifyException("Unable to delete published concepts");
                }
            }
        } else {
            throw new OpenLumifyException("Unable to delete concept that have children");
        }
    }
        ...
        if (property != null) {
            if (property.getSandboxStatus().equals(SandboxStatus.PRIVATE)) {
                ...
                if (results == 0) {
                    ...
                    throw new OpenLumifyException("Unable to delete property that have elements using it");
                }
            } else {
                throw new OpenLumifyException("Unable to delete published properties");
            }

        } else throw new OpenLumifyResourceNotFoundException("Property not found");
    }
            ...
            for (Relationship relationship : relationships) {
                if (relationship.getSandboxStatus().equals(SandboxStatus.PRIVATE)) {
                    ...
                    if (results == 0) {
                        ...
                        throw new OpenLumifyException("Unable to delete relationship that have edges using it");
                    }
                } else {
                    throw new OpenLumifyException("Unable to delete published relationships");
                }
            }
        } else throw new OpenLumifyException("Unable to delete relationship that have children");
    }
    ...
    public void addConceptTypeFilterToQuery(Query query, Collection<ElementTypeFilter> filters, String workspaceId) {
        ...
        query.has(OpenLumifyProperties.CONCEPT_TYPE.getPropertyName(), Contains.IN, conceptIds);
    }
    ...
    protected void checkPrivileges(User user, String workspaceId) {
        ...
        if (user == null) {
            throw new OpenLumifyAccessDeniedException("You must provide a valid user to perform this action", null, null);
        }

        if (isPublic(workspaceId)) {
            if (!getPrivilegeRepository().hasPrivilege(user, Privilege.ONTOLOGY_PUBLISH)) {
                throw new OpenLumifyAccessDeniedException("User does not have ONTOLOGY_PUBLISH privilege", user, null);
            }
        } else {
            ...
            if (!access) {
                throw new OpenLumifyAccessDeniedException("User does not have access to workspace", user, null);
            }

            if (!getPrivilegeRepository().hasPrivilege(user, Privilege.ONTOLOGY_ADD)) {
                throw new OpenLumifyAccessDeniedException("User does not have ONTOLOGY_ADD privilege", user, null);
            }
        }
    }
    ...
    protected void checkDeletePrivileges(User user, String workspaceId) {
        ...
        if (workspaceId == null) {
            throw new OpenLumifyAccessDeniedException("User does not have access to delete published ontology items", user, null);
        } else if (!getPrivilegeRepository().hasPrivilege(user, Privilege.ADMIN)) {
            throw new OpenLumifyAccessDeniedException("User does not have admin privilege", user, null);
        }
    }
    ...
    protected void validateRelationship(String relationshipIRI,
        ...
        if (!relationshipIRI.equals(TOP_OBJECT_PROPERTY_IRI)
            ...
            throw new OpenLumifyException(relationshipIRI + " must have at least one domain and range ");
        }
    }
}
