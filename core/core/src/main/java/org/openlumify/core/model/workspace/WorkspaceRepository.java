...
import org.openlumify.core.ingest.video.VideoFrameInfo;
...
import org.openlumify.core.model.workspace.product.*;
...
public abstract class WorkspaceRepository {
    ...
    public static final OpenLumifyVisibility VISIBILITY_PRODUCT = new OpenLumifyVisibility(VISIBILITY_PRODUCT_STRING);
    ...
    public static final String OWL_IRI = "http://openlumify.org/workspace";
    private static final OpenLumifyLogger LOGGER = OpenLumifyLoggerFactory.getLogger(WorkspaceRepository.class);
    ...
    private OpenLumifyResourceBundleManager openlumifyResourceBundleManager;
    private ResourceBundle openlumifyResourceBundle;
    ...
    protected WorkspaceRepository(
        ...
        openlumifyResourceBundleManager = new OpenLumifyResourceBundleManager(configuration);
        openlumifyResourceBundle = openlumifyResourceBundleManager.getBundle();
    ...
    public String getDefaultWorkspaceName(User user) {
        return openlumifyResourceBundle.getString("workspace.default.title").replace("{0}", user.getDisplayName());
    }
    ...
    private void publishRequiredConcepts(
        ...
        Map<String, List<String>> vertexIdsByConcept = StreamUtils.stream(verticesToPublish)
                .collect(Collectors.groupingBy(OpenLumifyProperties.CONCEPT_TYPE::getPropertyValue, Collectors.mapping(Vertex::getId, Collectors.toList())));
    ...
    private void publishProperties(
        ...
        for (ClientApiPublishItem data : publishData) {
            try {
                ...
                if (!ontologyProperty.getUserVisible() && !propertyName.equals(OpenLumifyProperties.COMMENT.getPropertyName())
                        || propertyName.equals(OpenLumifyProperties.ENTITY_IMAGE_VERTEX_ID.getPropertyName())) {
                    continue;
                }
    ...
    private void publishVertex(
        ...
        VisibilityJson visibilityJson = OpenLumifyProperties.VISIBILITY_JSON.getPropertyValue(vertex);
        ...
        vertexElementMutation.alterElementVisibility(openlumifyVisibility.getVisibility());
        ...
        for (Vertex termMention : termMentionRepository.findByVertexId(vertex.getId(), authorizations)) {
            termMentionRepository.updateVisibility(termMention, openlumifyVisibility.getVisibility(), authorizations);
        }
        ...
        for (int i = 0; i < properties.size(); i++) {
            ...
            if (WorkspaceDiffHelper.isPublicDelete(property, authorizations)) {
                ...
                if (publicProperty != null) {
                    ...
                    VisibilityJson visibilityJson = OpenLumifyProperties.VISIBILITY_JSON_METADATA.getMetadataValue(metadata);
                    VisibilityJson.removeFromWorkspace(visibilityJson, workspaceId);
                    OpenLumifyProperties.VISIBILITY_JSON_METADATA.setMetadata(
                            metadata,
                            visibilityJson,
                            visibilityTranslator.getDefaultVisibility()
                    );
    ...
    private boolean publishNewProperty(ExistingElementMutation elementMutation, Property property, String workspaceId) {
        VisibilityJson visibilityJson = OpenLumifyProperties.VISIBILITY_JSON_METADATA.getMetadataValue(property.getMetadata());
        ...
        elementMutation
                .alterPropertyVisibility(property, openlumifyVisibility.getVisibility())
                .setPropertyMetadata(
                        property,
    ...
    private void publishEdge(
        ...
        VisibilityJson visibilityJson = OpenLumifyProperties.VISIBILITY_JSON.getPropertyValue(edge);
        if (visibilityJson == null || !visibilityJson.getWorkspaces().contains(workspaceId)) {
            throw new OpenLumifyException(String.format(
        ...
        edgeExistingElementMutation.alterElementVisibility(openlumifyVisibility.getVisibility());

        for (Property property : edge.getProperties()) {
            boolean userVisible;
            if (OpenLumifyProperties.JUSTIFICATION.getPropertyName().equals(property.getName())) {
                userVisible = false;
            } else {
                OntologyProperty ontologyProperty = ontologyRepository.getPropertyByIRI(property.getName(), workspaceId);
                checkNotNull(
    ...
    private boolean shouldAutoPublishElementProperty(Property property, boolean userVisible) {
        ...
        if (propertyName.equals(OpenLumifyProperties.ENTITY_IMAGE_VERTEX_ID.getPropertyName())) {
            return false;
        }

        if (propertyName.equals(OpenLumifyProperties.COMMENT.getPropertyName())) {
            return false;
        }

        if (propertyName.equals(OpenLumifyProperties.CONCEPT_TYPE.getPropertyName())
                || propertyName.equals(OpenLumifyProperties.MODIFIED_BY.getPropertyName())
                || propertyName.equals(OpenLumifyProperties.MODIFIED_DATE.getPropertyName())
                || propertyName.equals(OpenLumifyProperties.VISIBILITY_JSON.getPropertyName())) {
            VisibilityJson visibilityJson = OpenLumifyProperties.VISIBILITY_JSON_METADATA.getMetadataValue(property.getMetadata());
            if (visibilityJson != null) {
    ...
    private void publishGlyphIconProperties(Edge hasImageEdge, String workspaceId, Authorizations authorizations) {
        ...
        Iterable<Property> glyphIconProperties = entityVertex.getProperties(OpenLumifyProperties.ENTITY_IMAGE_VERTEX_ID.getPropertyName());
    ...
    protected WorkProductService getWorkProductServiceByKind(String kind) {
        if (kind == null) {
            throw new OpenLumifyException("Work product kind must not be null");
        }
        if (workProductServices == null) {
            if (configuration == null) {
                throw new OpenLumifyException("Configuration not injected");
            } else {
        ...
        if (foundProductService.isPresent()) {
            ...
            throw new OpenLumifyException("Work Product of kind: " + kind + " not found");
        }
    }
