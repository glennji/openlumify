package org.openlumify.core.model.textHighlighting;
...
import org.openlumify.core.util.SourceInfoSnippetSanitizer;
...
public class VertexOffsetItem extends OffsetItem {
    ...
    public long getStart() {
        long start = OpenLumifyProperties.TERM_MENTION_START_OFFSET.getPropertyValue(termMention, 0);
    ...
    @Override
    public long getEnd() {
        long end = OpenLumifyProperties.TERM_MENTION_END_OFFSET.getPropertyValue(termMention, 0);
        if (shouldBitShiftOffsetsForVideoTranscript) {
    ...
    public String getConceptIri() {
        return OpenLumifyProperties.TERM_MENTION_CONCEPT_TYPE.getPropertyValue(termMention);
    }

    public String getSnippet() {
        return SourceInfoSnippetSanitizer.sanitizeSnippet(
                OpenLumifyProperties.TERM_MENTION_SNIPPET.getPropertyValue(termMention, null)
        );
    }
    ...
    public String getProcess() {
        String process = OpenLumifyProperties.TERM_MENTION_PROCESS.getPropertyValue(termMention);
        if (process == null) {
            return null;
        }
    ...
    public String getOutVertexId() {
        return singleOrDefault(termMention.getVertexIds(Direction.IN, OpenLumifyProperties.TERM_MENTION_LABEL_HAS_TERM_MENTION, this.authorizations), null);
    }

    @Override
    public String getResolvedToVertexId() {
        return singleOrDefault(termMention.getVertexIds(Direction.OUT, OpenLumifyProperties.TERM_MENTION_LABEL_RESOLVED_TO, this.authorizations), null);
    }

    @Override
    public String getResolvedFromTermMentionId() {
        return singleOrDefault(termMention.getVertexIds(Direction.OUT, OpenLumifyProperties.TERM_MENTION_RESOLVED_FROM, this.authorizations), null);
    }

    @Override
    public String getResolvedToTermMentionId() {
        return singleOrDefault(termMention.getVertexIds(Direction.IN, OpenLumifyProperties.TERM_MENTION_RESOLVED_FROM, this.authorizations), null);
    }

    @Override
    public String getResolvedToEdgeId() {
        return OpenLumifyProperties.TERM_MENTION_RESOLVED_EDGE_ID.getPropertyValue(termMention);
    }

    @Override
    public TermMentionFor getTermMentionFor() {
        return OpenLumifyProperties.TERM_MENTION_FOR_TYPE.getPropertyValue(termMention);
    }

    @Override
    public String getTermMentionForElementId() {
        return OpenLumifyProperties.TERM_MENTION_FOR_ELEMENT_ID.getPropertyValue(termMention);
    }
    ...
    public String getTitle() {
        return OpenLumifyProperties.TERM_MENTION_TITLE.getPropertyValue(termMention);
    }
