...
public class OWLOntologyUtil {
    public static final String OBJECT_PROPERTY_DOMAIN_IRI = "http://openlumify.org#objectPropertyDomain";
    public static final String EXTENDED_DATA_TABLE_DOMAIN_IRI = "http://openlumify.org#extendedDataTableDomain";
    public static final String EXTENDED_DATA_TABLE_IRI = "http://openlumify.org#extendedDataTable";
    ...
    public static PropertyType getPropertyType(OWLOntology o, OWLDataProperty dataTypeProperty) {
        ...
        if (ranges.size() > 1) {
            throw new OpenLumifyException("Unexpected number of ranges on data property " + dataTypeProperty.getIRI().toString());
        }
        ...
        throw new OpenLumifyException("Could not find range on data property " + dataTypeProperty.getIRI().toString());
    }
    ...
    public static PropertyType getPropertyType(String iri) {
        ...
        if ("http://openlumify.org#geolocation".equals(iri)) {
            return PropertyType.GEO_LOCATION;
        }
        if ("http://openlumify.org#geoshape".equals(iri)) {
            return PropertyType.GEO_SHAPE;
        }
        if ("http://openlumify.org#directory/entity".equals(iri)) {
            return PropertyType.DIRECTORY_ENTITY;
        }
        if ("http://openlumify.org#currency".equals(iri)) {
            return PropertyType.CURRENCY;
        }
        if ("http://openlumify.org#image".equals(iri)) {
            return PropertyType.IMAGE;
        }
        if ("http://openlumify.org#extendedDataTable".equals(iri)) {
        ...
        throw new OpenLumifyException("Unhandled property type " + iri);
    }
