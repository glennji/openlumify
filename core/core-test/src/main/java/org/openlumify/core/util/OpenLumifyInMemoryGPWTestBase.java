...
import org.openlumify.core.ingest.graphProperty.*;
...
public class OpenLumifyInMemoryGPWTestBase extends OpenLumifyInMemoryTestBase {
    private User user;
    ...
    protected boolean run(
        ...
        try {
            ...
            throw new OpenLumifyException("Failed to prepare: " + gpw.getClass().getName(), ex);
        }
        ...
        try {
            ...
            throw new OpenLumifyException("Failed isHandled: " + gpw.getClass().getName(), ex);
        }
        ...
        try {
            ...
            if (gpw.isLocalFileRequired() && workData.getLocalFile() == null && in != null) {
                ...
                File tempFile = File.createTempFile("openlumifyTest", "data");
            ...
            throw new OpenLumifyException("Failed to execute: " + gpw.getClass().getName(), ex);
        }
        return true;
    }
    ...
    private String getVisibilitySource(Element e) {
        ...
        if (e != null) {
            VisibilityJson visibilitySourceJson = OpenLumifyProperties.VISIBILITY_JSON.getPropertyValue(e, null);
    ...
    protected GraphPropertyWorkerPrepareData createWorkerPrepareData(
        ...
        if (authorizations == null) {
            authorizations = getGraphAuthorizations(user, OpenLumifyVisibility.SUPER_USER_VISIBILITY_STRING);
        }
