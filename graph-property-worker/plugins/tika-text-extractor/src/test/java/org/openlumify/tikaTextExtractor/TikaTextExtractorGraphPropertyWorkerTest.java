...
public class TikaTextExtractorGraphPropertyWorkerTest extends OpenLumifyInMemoryGPWTestBase {
    private static final String DOCUMENT_TITLE_PROPERTY_IRI = "http://openlumify.org/test#title";
    ...
    protected Map getConfigurationMap() {
        ...
        result.put(TikaTextExtractorGraphPropertyWorkerConfiguration.TEXT_EXTRACT_MAPPING_CONFIGURATION_PREFIX + ".text1.rawPropertyName", "http://openlumify.org/test#raw1");
        result.put(TikaTextExtractorGraphPropertyWorkerConfiguration.TEXT_EXTRACT_MAPPING_CONFIGURATION_PREFIX + ".text1.extractedTextPropertyName", "http://openlumify.org/test#text1");
        ...
        result.put(TikaTextExtractorGraphPropertyWorkerConfiguration.TEXT_EXTRACT_MAPPING_CONFIGURATION_PREFIX + ".text2.rawPropertyName", "http://openlumify.org/test#raw2");
        result.put(TikaTextExtractorGraphPropertyWorkerConfiguration.TEXT_EXTRACT_MAPPING_CONFIGURATION_PREFIX + ".text2.extractedTextPropertyName", "http://openlumify.org/test#text2");
    ...
    public void testExtractWithHtml() throws Exception {
        ...
        assertEquals(
                ...
                IOUtils.toString(OpenLumifyProperties.TEXT.getOnlyPropertyValue(vertex).getInputStream(), "UTF-8")
        );
        assertEquals(new Date(1357063760000L), OpenLumifyProperties.MODIFIED_DATE.getPropertyValue(vertex));
    }
    ...
    private void createVertex(String data, String mimeType) throws UnsupportedEncodingException {
        ...
        metadata.add(OpenLumifyProperties.MIME_TYPE.getPropertyName(), mimeType, getVisibilityTranslator().getDefaultVisibility());
        OpenLumifyProperties.RAW.setProperty(v, textValue, metadata, visibility);
    ...
    public void testExtractWithEmptyHtml() throws Exception {
        ...
        vertex = getGraph().getVertex("v1", getGraphAuthorizations());
        assertEquals("Test Title", vertex.getPropertyValue(DOCUMENT_TITLE_PROPERTY_IRI));
        assertEquals("", IOUtils.toString(OpenLumifyProperties.TEXT.getOnlyPropertyValue(vertex).getInputStream(), "UTF-8"));
        assertEquals(new Date(1357063760000L), OpenLumifyProperties.MODIFIED_DATE.getPropertyValue(vertex));
    }
    ...
    public void testExtractTextWithAccentCharacters() throws Exception {
        ...
        String actual = IOUtils.toString(OpenLumifyProperties.TEXT.getOnlyPropertyValue(vertex).getInputStream(), "UTF-8");
    ...
    public void testDifferentKey() throws UnsupportedEncodingException {
        ...
        textValue.searchIndex(false);
        Metadata metadata = new Metadata();
        metadata.add(OpenLumifyProperties.MIME_TYPE.getPropertyName(), "text/html", getVisibilityTranslator().getDefaultVisibility());
        v.addPropertyValue("key1", "http://openlumify.org/test#raw1", textValue, metadata, visibility);
        ...
        Property text1Property = vertex.getProperty("key1", "http://openlumify.org/test#text1");
    ...
    public void testMultipleRaws() throws UnsupportedEncodingException {
        ...
        Metadata metadata = new Metadata();
        metadata.add(OpenLumifyProperties.MIME_TYPE.getPropertyName(), "text/html", getVisibilityTranslator().getDefaultVisibility());
        v.setProperty("http://openlumify.org/test#raw1", textValue, metadata, visibility);

        textValue = StreamingPropertyValue.create(new ByteArrayInputStream("<html><body>Text2</body></html>".getBytes("UTF-8")), byte[].class);
        textValue.searchIndex(false);
        metadata = new Metadata();
        metadata.add(OpenLumifyProperties.MIME_TYPE.getPropertyName(), "text/html", getVisibilityTranslator().getDefaultVisibility());
        v.setProperty("http://openlumify.org/test#raw2", textValue, metadata, visibility);
        ...
        Property text1Property = vertex.getProperty("http://openlumify.org/test#text1");
        assertTrue(text1Property != null);
        assertThat(((StreamingPropertyValue) text1Property.getValue()).readToString(), equalTo("Text1"));
        assertThat(OpenLumifyProperties.TEXT_DESCRIPTION_METADATA.getMetadataValue(text1Property.getMetadata()), equalTo("Text 1"));

        Property text2Property = vertex.getProperty("http://openlumify.org/test#text2");
        assertTrue(text2Property != null);
        assertThat(((StreamingPropertyValue) text2Property.getValue()).readToString(), equalTo("Text2"));
        assertThat(OpenLumifyProperties.TEXT_DESCRIPTION_METADATA.getMetadataValue(text2Property.getMetadata()), equalTo(null));
    }
