package org.openlumify.tools.ontology.ingest.common;
...
public class IngestRepository {
    private static final OpenLumifyLogger LOGGER = OpenLumifyLoggerFactory.getLogger(IngestRepository.class);
        ...
        if (ingestOptions.isValidateOntologyWhenSaving()) {
            for (EntityBuilder builder : builders) {
                ...
                if (!validationResult.isValid()) {
                    throw new OpenLumifyException(validationResult.getValidationError());
                }
            }
        }
    ...
    private void addProperties(
        ...
        OpenLumifyProperties.MODIFIED_BY.setProperty(elementBuilder, ingestOptions.getIngestUser().getUserId(), visibilityTranslator.getDefaultVisibility());
        OpenLumifyProperties.MODIFIED_DATE.setProperty(elementBuilder, new Date(), visibilityTranslator.getDefaultVisibility());
        OpenLumifyProperties.VISIBILITY_JSON.setProperty(elementBuilder, new VisibilityJson(elementVisibility.getVisibilityString()), visibilityTranslator.getDefaultVisibility());
    ...
    private Metadata buildMetadata(IngestOptions ingestOptions, Map<String, Object> map, String visibilitySource) {
        ...
        OpenLumifyProperties.MODIFIED_BY_METADATA.setMetadata(
                metadata,
                ingestOptions.getIngestUser().getUserId(),
                defaultVisibility
        );
        OpenLumifyProperties.CONFIDENCE_METADATA.setMetadata(
                metadata,
