...
public class VertexGetPropertyValue implements ParameterizedHandler {
    ...
    public void handle(
        ...
        String fileName = OpenLumifyProperties.FILE_NAME.getOnlyPropertyValue(vertex);
    ...
    private void setFileNameHeaders(OpenLumifyResponse response, String fileName, PlaybackOptions playbackOptions) {
    ...
    private void handleFullPlayback(OpenLumifyResponse response, InputStream in) throws IOException {
        try (OutputStream out = response.getOutputStream()) {
            IOUtils.copy(in, out);
        }
    }

    private void handlePartialPlayback(OpenLumifyResponse response, InputStream in, long totalLength, PlaybackOptions playbackOptions) throws IOException {
        long partialStart = 0;
