...
import org.openlumify.core.process.OpenLumifyProcess;
import org.openlumify.core.process.OpenLumifyProcessOptions;
...
import org.openlumify.web.auth.AuthTokenFilter;
import org.openlumify.web.auth.AuthTokenWebSocketInterceptor;
...
public class ApplicationBootstrap implements ServletContextListener {
    private static OpenLumifyLogger LOGGER;
    ...
    public static final String VISALLO_SERVLET_NAME = "openlumify";
    ...
    public void contextInitialized(ServletContextEvent sce) {
        try {
            ...
            if (context == null) {
                throw new RuntimeException("Failed to initialize context. OpenLumify is not running.");
            }
            OpenLumifyLoggerFactory.setProcessType("web");
            ...
            LOGGER = OpenLumifyLoggerFactory.getLogger(ApplicationBootstrap.class);
            ...
            startOpenLumifyProcesses(config);
            ...
            throw new OpenLumifyException("Could not startup context", ex);
        }
    }

    private void startOpenLumifyProcesses(Configuration config) {
        boolean enableWebContainerProcesses = config.getBoolean("org.openlumify.web.ApplicationBootstrap.enableWebContainerProcesses", true);
        if (!enableWebContainerProcesses) {
            return;
        }

        Iterable<OpenLumifyProcess> processes = ServiceLoaderUtil.load(OpenLumifyProcess.class, config);
        User user = InjectHelper.getInstance(UserRepository.class).getSystemUser();
        OpenLumifyProcessOptions options = new OpenLumifyProcessOptions(user);
        for (OpenLumifyProcess process : processes) {
    ...
    private void setupInjector(ServletContext context, Configuration config) {
        ...
        InjectHelper.inject(this, OpenLumifyBootstrap.bootstrapModuleMaker(config), config);
