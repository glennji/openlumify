package org.openlumify.web.util.js;
...
public class BabelExecutor {
    private static final OpenLumifyLogger LOGGER = OpenLumifyLoggerFactory.getLogger(BabelExecutor.class);
    ...
    public synchronized void compileWithSharedEngine(CachedCompilation cachedCompilation, SourceMapType sourceMapType) throws ScriptException {
        ...
        if (result.containsKey("error")) {
            throw new OpenLumifyException((String) result.get("error"));
        }
    ...
    private String getTransformJavaScript() {
        ...
        try (StringWriter writer = new StringWriter()) {
            ...
            throw new OpenLumifyException("Unable to read babel transformer");
        }
        if (transform == null) {
            throw new OpenLumifyException("Babel configuration not found");
        }
        return transform;
    }
    ...
    private void initializeBabel() {
        try {
            ...
            if (engine == null) {
                throw new OpenLumifyException("JavaScript Engine \"nashorn\" not found. Unable to compile jsx");
            }
            ...
            throw new OpenLumifyException("Unable to initialize babel transpiler", e);
        }
    }
