define([
     ...
     */
    registry.documentExtensionPoint('org.openlumify.map.options',
        'Add components to the map options toolbar',
        function(e) {
            return ('identifier' in e) && ('optionComponentPath' in e);
        },
        'http://docs.openlumify.org/extension-points/front-end/mapOptions'
    );

    registry.markUndocumentedExtensionPoint('org.openlumify.map.style');

    registry.markUndocumentedExtensionPoint('org.openlumify.map.geometry');

    registry.markUndocumentedExtensionPoint('org.openlumify.map.layer');
    ...
    const Map = createReactClass({
        ...
        componentDidMount() {
            ...
            $(document).on('elementsCut.org-openlumify-map', (event, { vertexIds }) => {
                this.props.onRemoveElementIds({ vertexIds, edgeIds: [] });
            })
            $(document).on('elementsPasted.org-openlumify-map', (event, elementIds) => {
        ...
        componentWillUnmount() {
            ...
            $(document).off('.org-openlumify-map');
        ...
        render() {
            ...
            const layerExtensions = _.indexBy(registry['org.openlumify.map.layer'], 'id');

            return (
                <div className="org-openlumify-map" style={{height:'100%'}} ref={r => {this.wrap = r}}>
                    <OpenLayers
        ...
        getGeometry(edgeInfo, element, ontology) {
            ...
            const calculatedGeometry = registry['org.openlumify.map.geometry']
                .reduce((geometries, { canHandle, geometry, layer }) => {
                    /**
                     * Decide which elements to apply geometry
                     *
                     * @function org.openlumify.map.geometry~canHandle
                    ...
                    if (this.caches.geometries.canHandle.getOrUpdate(canHandle, edgeInfo, element, ontology)) {
                         ...
                         *
                         * @function org.openlumify.map.geometry~geometry
                         * @param {object} productEdgeInfo The edge info from product->vertex
                         * @param {object} element The element
                         * @param {Array.<object>} element.properties The element properties
                        ...
                        if (geo) {
                             ...
                             *
                             * @typedef org.openlumify.map.geometry~layer
        ...
        getStyles(edgeInfo, element, ontology) {
            const { registry } = this.props;
            const calculatedStyles = registry['org.openlumify.map.style']
                .reduce((styles, { canHandle, style, selectedStyle }) => {

                    /**
                     * Decide which elements to apply style
                     *
                     * @function org.openlumify.map.style~canHandle
                     * @param {object} productEdgeInfo The edge info from product->vertex
                     * @param {object} element The element
                     * @param {Array.<object>} element.properties The element properties
                     * @param {object} ontology The ontology object for this element (concept/relationship)
                    ...
                    if (this.caches.styles.canHandle.getOrUpdate(canHandle, edgeInfo, element, ontology)) {
                        if (style) {
                             ...
                             * object for the given element.
                             *
                             * @function org.openlumify.map.style~style
                             * @param {object} productEdgeInfo The edge info from product->vertex
                             * @param {object} element The element
                             * @param {Array.<object>} element.properties The element properties
        ...
        mapElementsToSources() {
            ...
            elements.forEach(el => {
                ...
                if (F.vertex.displayType(el) === 'document') {
                    const mimeType = F.vertex.prop(el, 'http://openlumify.org#mimeType');

                    if (GEOSHAPE_MIMETYPES.includes(mimeType)) {
                        const rawProp = F.vertex.props(el, 'http://openlumify.org#raw')[0];
                        addOrUpdateSource({
                            id: el.id,
                            element: el,
                            type: 'vectorXhr',
    ...
    return RegistryInjectorHOC(Map, [
        'org.openlumify.map.style',
        'org.openlumify.map.geometry',
        'org.openlumify.map.layer'
    ])
});
