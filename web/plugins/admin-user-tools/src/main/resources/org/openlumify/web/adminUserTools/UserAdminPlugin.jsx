define([
    ...
    const UserAdminPlugin = createReactClass({
        ...
        componentWillMount() {
            ...
            const privilegesPlugins = openlumify.registry.extensionsForPoint('org.openlumify.admin.user.privileges');
            if (privilegesPlugins.length != 1) {
                throw new Error('Exactly one "org.openlumify.admin.user.privileges" is required. Found: ' + privilegesPlugins.length);
            }

            const authorizationsPlugins = openlumify.registry.extensionsForPoint('org.openlumify.admin.user.authorizations');
            if (authorizationsPlugins.length != 1) {
                throw new Error('Exactly one "org.openlumify.admin.user.authorizations" is required. Found: ' + authorizationsPlugins.length);
            }
        ...
        handleUserLoaded(user) {
            this.setState({
                user: user,
                reloadUser: false,
                disableDelete: user.id === openlumifyData.currentUser.id
            });
        },
