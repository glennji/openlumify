define([
    ...
    const Graph = createReactClass({
        ...
        componentWillReceiveProps(nextProps) {
            ...
            if (nextProps.concepts !== this.props.concepts || nextProps.properties !== this.props.properties) {
                clearForOntologyUpdates.push(
                    'vertexToCyNode',
                    'org.openlumify.graph.node.class',
                    'org.openlumify.graph.node.transformer',
                    'org.openlumify.graph.node.decoration#applyTo',
                    'org.openlumify.graph.node.decoration#data'
                )
            }
            if (nextProps.relationships !== this.props.relationships) {
                clearForOntologyUpdates.push(
                    'vertexToCyNode',
                    'org.openlumify.graph.edge.transformer',
                    'org.openlumify.graph.edge.class')
            }
        ...
        render() {
            var { draw, paths } = this.state,
                ...
                extensionViews = registry['org.openlumify.graph.view'];

            return (
                <div ref={r => {this.node = r}} className="org-openlumify-graph" style={{ height: '100%' }}>
                    <Cytoscape
        ...
        onFocusPaths(event, data) {
            ...
            if (data.paths.length > MaxPathsToFocus) {
                ...
                $(document).trigger('displayInformation', {
                    message: i18n('org.openlumify.web.product.graph.findPath.too.many', MaxPathsToFocus)
                })
            }
        ...
        onDecorationEvent(event) {
            ...
            if (decoration) {
                const handlerName = {
                    /**
                     * @callback org.openlumify.graph.node.decoration~onClick
                    ...
                    tap: 'onClick',
                    /**
                     * @callback org.openlumify.graph.node.decoration~onMouseOver
                     * @this The decoration cytoscape node
                     * @param {object} event The {@link http://js.cytoscape.org/#events/event-object|Cytoscape event} object
                     * @param {object} data
                     * @param {object} data.vertex The vertex this decoration
                    ...
                    /**
                     * @callback org.openlumify.graph.node.decoration~onMouseOut
                     * @this The decoration cytoscape node
                     * @param {object} event The {@link http://js.cytoscape.org/#events/event-object|Cytoscape event} object
                     * @param {object} data
                     * @param {object} data.vertex The vertex this decoration
        ...
        onMenuSelect(select) {
            ...
            switch (select) {
                ...
                case 'none':
                    ...
                    return;
                default:
                    const selector = registry['org.openlumify.graph.selection'].find(e => e.identifier === select);
                    if (selector) {
        ...
        onMenuExport(componentPath) {
            var exporter = _.findWhere(
                    this.props.registry['org.openlumify.graph.export'],
                    { componentPath }
                );
        ...
        onEditCollapsedNode(collapsedNodeId) {
            ...
            if (this.props.workspace.editable && collapsedNode) {
                Promise.require('org/openlumify/web/product/graph/popovers/collapsedNode/collapsedNodePopoverShim')
        ...
        mapPropsToElements(editable) {
            ...
            const cyNodeConfig = (node) => {
                ...
                if (ancillary) {
                    ...
                    selectable = false;
                    classes = 'ancillary';
                    registry['org.openlumify.graph.ancillary'].forEach(({
                    ...
                    classes = mapVertexToClasses(id, vertices, focusing, hovering, registry['org.openlumify.graph.node.class']);
                    data = mapVertexToData(id, vertices, registry['org.openlumify.graph.node.transformer']);
                    ...
                    classes = mapCollapsedNodeToClasses(id, collapsedNodes, focusing, vertexIds, hovering, registry['org.openlumify.graph.collapsed.class']);
            ...
            const cyVertices = filterByRoot(productVertices).reduce((nodes, nodeData) => {
                ...
                if (id in vertices) {
                    ...
                    const applyDecorations = !ancillary && memoizeFor('org.openlumify.graph.node.decoration#applyTo', vertex, () => {
                        return _.filter(registry['org.openlumify.graph.node.decoration'], function(e) {
                            /**
                             * @callback org.openlumify.graph.node.decoration~applyTo
                             * @param {object} vertex
                    ...
                    if (applyDecorations && applyDecorations.length) {
                        ...
                        const decorations = memoizeFor('org.openlumify.graph.node.decoration#data', vertex, () => {
                            return applyDecorations.map((dec, index) => {
                                const data = mapDecorationToData(dec, vertex, data => {
                                    if (data) {
                                        const cache = memoizeGet('org.openlumify.graph.node.decoration#data', vertex);
                    ...
                    if (ghosts) {
                        _.mapObject(ghosts, (ghost, ghostId) => {
                            if (cyNode.data.vertexIds.includes(ghostId)) {
                                const ghostData = {
                                    ...mapVertexToData(ghostId, vertices, registry['org.openlumify.graph.node.transformer']),
                                    parent: rootNode.id,
                                ...
                                nodes.push({
                                    ...cyNode,
                                    data: ghostData,
                                    classes: mapVertexToClasses(ghostId, vertices, focusing, hovering, registry['org.openlumify.graph.node.class']),
            ...
            const cyEdges = _.chain(productEdges)
                ...
                .reduce((edgeGroups, edgeGroup) => {
                    if (edgeGroup.length > MaxEdgesBetween) {
                        ...
                        const multiEdgeLabel = (edges) => {
                            ...
                            return numTypes === 1 ?
                                i18n('org.openlumify.web.product.graph.multi.edge.label.single.type', edges.length, display) :
                                i18n('org.openlumify.web.product.graph.multi.edge.label', edges.length, numTypes);
                        }
                        let classes = 'e';
                ...
                .map(data => {
                    if (data.id) {
                        ...
                        return {
                            data: mapEdgeToData(data, edgesForInfos, relationships, registry['org.openlumify.graph.edge.transformer']),
                            classes: mapEdgeToClasses(data.edgeInfos, edgesForInfos, focusing, registry['org.openlumify.graph.edge.class']),
    ...
    const mapEdgeToData = (data, edges, ontologyRelationships, transformers) => {
        ...
        return memoizeFor('org.openlumify.graph.edge.transformer', edges, () => {
            ...
            if (edges.length) {
                return transformers.reduce((data, fn) => {
                     ...
                     *
                     * @callback org.openlumify.graph.edge.transformer~transformerFn
    ...
    const mapEdgeToClasses = (edgeInfos, edges, focusing, classers) => {
        ...
        if (edges.length) {
             ...
             *
             * @callback org.openlumify.graph.edge.class~classFn
            ...
            cls = memoizeFor('org.openlumify.graph.edge.class', edges, function() {
                const cls = [];
    ...
    const mapDecorationToData = (decoration, vertex, update) => {
        const getData = () => {
             ...
             *
             * @callback org.openlumify.graph.node.decoration~data
             * @param {object} vertex
    ...
    const mapDecorationToClasses = (decoration, vertex) => {
        ...
        if (_.isString(decoration.classes)) {
            ...
            /**
             * @callback org.openlumify.graph.node.decoration~classes
             * @param {object} vertex
    ...
    const mapVertexToClasses = (id, vertices, focusing, hovering, classers) => {
        ...
        if (vertexLoaded) {
             ...
             * Mutate the classes array to adjust the classes.
             *
             * @callback org.openlumify.graph.node.class~classFn
             ...
             * }
             */
            cls = memoizeFor('org.openlumify.graph.node.class', vertex, function() {
                const cls = [];
    ...
    const mapCollapsedNodeToClasses = (collapsedNodeId, collapsedNodes, focusing, vertexIds, hovering, classers) => {
        ...
        if (loaded) {
             ...
             * Mutate the classes array to adjust the classes.
             *
             * @callback org.openlumify.graph.collapsed.class~classFn
    ...
    const generateCollapsedNodeTitle = (collapsedNode, vertices, productVertices, collapsedNodes) => {
        ...
        if (vertices) {
            ...
            if (vertex && vertex.length > 1) {
                ...
                title = i18n('org.openlumify.web.product.graph.collapsedNode.entities.singular');
            }
        }

        return title || i18n('org.openlumify.web.product.graph.collapsedNode.entities', children.length);
    };
    ...
    const vertexToCyNode = (vertex, transformers) => {
        return memoizeFor('vertexToCyNode', vertex, function() {
            ...
            return transformers.reduce((data, t) => {
                 ...
                 *
                 * @callback org.openlumify.graph.node.transformer~transformerFn
    ...
    const CONFIGURATION = (props) => {
        ...
        const styleExtensions = registry['org.openlumify.graph.style'];
    ...
    return RegistryInjectorHOC(Graph, [
        'org.openlumify.graph.edge.class',
        'org.openlumify.graph.edge.transformer',
        'org.openlumify.graph.export',
        'org.openlumify.graph.node.class',
        'org.openlumify.graph.node.decoration',
        'org.openlumify.graph.node.transformer',
        'org.openlumify.graph.ancillary',
        'org.openlumify.graph.collapsed.class',
        'org.openlumify.graph.selection',
        'org.openlumify.graph.style',
        'org.openlumify.graph.view'
    ]);
});
