    ...
    const FindPathTopLevelConfig = createReactClass({
        ...
        render(){
            ...
            const edgesLabel = this.props.availableEdges.length === count
                ? i18n('org.openlumify.web.product.graph.findPath.edgesAll')
                : arraysContainsSameItems(this.props.selectedEdgeIris, this.props.defaultSelectedEdgeIris)
                    ? i18n('org.openlumify.web.product.graph.findPath.edgesDefault')
                    : count === 1
                    ? i18n('org.openlumify.web.product.graph.findPath.edgesCount.single', count)
                    : i18n('org.openlumify.web.product.graph.findPath.edgesCount', count);

            return (<div className="find-path-config">
                <div className="field">
                    <div className="field-title-value">
                        <div className="field-title">{i18n('org.openlumify.web.product.graph.findPath.edges')}</div>
                        <div className="value" onClick={this.handleEdgesValueClick}>{edgesLabel}</div>
                    </div>
                    <div className="subtitle">{i18n('org.openlumify.web.product.graph.findPath.edgesSubtitle')}</div>
                </div>
                <div className="field">
                    <div className="field-title-value">
                        <div className="field-title">{i18n('org.openlumify.web.product.graph.findPath.maximumHops')}</div>
                        <div className="value">
                    ...
                    <div className="subtitle">{i18n('org.openlumify.web.product.graph.findPath.maximumHopsSubtitle')}</div>

                    <FindPathHopsImage
                        hops={this.props.maximumHops}
                        hopsTitle={i18n('org.openlumify.web.product.graph.findPath.maximumHopsImageAltText', this.props.maximumHops)}
                ...
                <div>
                    <button
                        ...
                        onClick={this.handleExecuteClick}>{i18n('org.openlumify.web.product.graph.findPath.execute')}</button>
                </div>
            </div>);
        }
    });
