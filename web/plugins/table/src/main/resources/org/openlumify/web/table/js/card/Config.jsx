define([
    ...
    const Config = createReactClass({
        ...
        componentWillMount() {
            this.props.openlumifyApi.v1.dataRequest('search', 'all')
                .then((searches) => {
                    this.setState({
        ...
        render() {
            ...
            return (
                <section>
                    <label>{i18n('com.openlumify.table.config.search.label')}</label>
                    <select
                        className="search"
                    ...
                    >
                        <option
                            ...
                            value={''}
                        >
                            {i18n('com.openlumify.table.config.search.placeholder')}
                        </option>
                        {savedSearches.map(
