define([
    ...
    const AGGREGATION_FIELD = {
        vertex: 'http://openlumify.org#conceptType',
        edge: '__edgeLabel'
    };
    ...
    const SavedSearchTableCard = createReactClass({
        ...
        componentWillMount() {
            this.dataRequest = this.props.openlumifyApi.v1.dataRequest;
        },
        ...
        render() {
            ...
            return (
                <div
                    className="com-openlumify-table"
                    ref={(ref) => {this.cardRef = ref}}
                >
                    {(() => {
                        if (!searchId) {
                            return <a onClick={configureItem}>{i18n('com.openlumify.table.config.search')}</a>
                        } else if (isLoading(searchId, tableView, tableData)) {
                            const message = isEmpty(searchId, tableData)
                                ? i18n('com.openlumify.table.watermark.empty')
                                : i18n('com.openlumify.table.watermark.loading');
        ...
        loadRows(startIndex, stopIndex) {
            ...
            function transformResultsToRows(results) {
                ...
                const F = self.props.openlumifyApi.v1.formatters;
                    ...
                    return Promise.map(columns, ({ title }) => {
                        ...
                        function getColumnValues(properties) {
                            return Promise.map(properties, (p) => {
                                ...
                                if (p.name === 'http://openlumify.org#title') {
    ...
    function transformItemToTable(iri, type, ontology) {
        ...
        return _.chain(item.properties)
            ...
            .sortBy((p) => {
                if (p.title === 'http://openlumify.org#title' || !p.displayName) {
