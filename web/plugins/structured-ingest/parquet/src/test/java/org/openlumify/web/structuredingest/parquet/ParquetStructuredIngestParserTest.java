...
import org.openlumify.web.structuredingest.core.util.mapping.ColumnMappingType;
...
public class ParquetStructuredIngestParserTest {
    private Graph graph;
    private static final String SSN_PROP_IRI = "http://openlumify.org/testontology#ssn";
    ...
    public void before() {
        ...
        OpenLumifyProperties.VISIBILITY_JSON.setProperty(structuredVertexBuilder, new VisibilityJson(), Visibility.EMPTY);
        ...
        when(ontologyRepository.getPropertyByIRI(OpenLumifyProperties.CONCEPT_TYPE.getPropertyName(), WORKSPACE_ID_A)).thenReturn(ontologyProperty);
        when(ontologyRepository.getPropertyByIRI(OpenLumifyProperties.CONCEPT_TYPE.getPropertyName(), WORKSPACE_ID_B)).thenReturn(ontologyProperty);
    ...
    private void assertConceptTypeOfAllVertices(Iterable<Vertex> vertices, String... conceptTypes) {
        for (Vertex v : vertices) {
            assertThat(OpenLumifyProperties.CONCEPT_TYPE.getPropertyValue(v), is(new org.hamcrest.BaseMatcher<String>() {
                @Override
    ...
    private static JSONObject conceptTypeObj(String conceptType) {
        return new JSONObject()
                .put(PropertyMapping.PROPERTY_MAPPING_NAME_KEY, OpenLumifyProperties.CONCEPT_TYPE.getPropertyName())
