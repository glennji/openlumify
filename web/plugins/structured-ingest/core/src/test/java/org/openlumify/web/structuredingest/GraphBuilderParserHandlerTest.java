package org.openlumify.web.structuredingest;
...
import static org.openlumify.web.structuredingest.mapping.MappingTestHelpers.createIndexedMap;

@SuppressWarnings("ConstantConditions")
public class GraphBuilderParserHandlerTest extends OpenLumifyInMemoryTestBase {
    private static final String WORKSPACE_ID = "testWorkspaceId";

    private static final String OWL_BASE_URI = "http://openlumify.org/structured-file-test";
    ...
    public void setup() throws Exception {
        ...
        getOntologyRepository().importFileData(
                inFileData,
                IRI.create("http://openlumify.org/structured-file-test"),
                null,
                authorizations
        );
        ...
        OpenLumifyProperties.VISIBILITY_JSON.setProperty(structuredFileVertexBuilder, visibilityJson, new Visibility(""));
    ...
    public void testAddRow() throws Exception {
        ...
        for (Vertex vertex : generated) {
            String conceptType = OpenLumifyProperties.CONCEPT_TYPE.getPropertyValue(vertex);
            ...
            if (PERSON_CONCEPT_TYPE.equals(conceptType)) {
                ...
                List<Property> property = Lists.newArrayList(vertex.getProperties(OpenLumifyProperties.VISIBILITY_JSON.getPropertyName()));
                ...
                assertEquals(
                        ...
                        TX_CONCEPT_TYPE,
                        OpenLumifyProperties.CONCEPT_TYPE.getPropertyValue(vertex)
                );
                assertEquals(
    ...
    public void testAddRowMultipleTimes() throws Exception {
        ...
        Predicate<? super Vertex> hasOneConceptType = vertex -> {
            List<Property> properties = Lists.newArrayList(vertex.getProperties(OpenLumifyProperties.CONCEPT_TYPE.getPropertyName()));
    ...
    public void testAddRowWithErrorThatSkipsCell() throws Exception {
        ...
        Optional<Vertex> txVertexOpt = getGenerated().stream()
                .filter(vertex -> OpenLumifyProperties.CONCEPT_TYPE.getPropertyValue(vertex).equals(TX_CONCEPT_TYPE))
                .findFirst();
    ...
    public void testAddRowWithErrorThatSkipsVertex() throws Exception {
        ...
        assertTrue("Unable to find new person vertex",
                getGenerated().stream()
                    .anyMatch(vertex -> OpenLumifyProperties.CONCEPT_TYPE.getPropertyValue(vertex).equals(PERSON_CONCEPT_TYPE)));

        assertFalse("Should not have found the transaction vertex",
                getGenerated().stream()
                    .anyMatch(vertex -> OpenLumifyProperties.CONCEPT_TYPE.getPropertyValue(vertex).equals(TX_CONCEPT_TYPE)));
    }
    ...
    public void testAddRowWithTooManyErrorsButNotDryRun() throws Exception {
        ...
        assertEquals(
                "Incorrect concept type on tx vertex",
                TX_CONCEPT_TYPE,
                OpenLumifyProperties.CONCEPT_TYPE.getPropertyValue(txVertex)
        );
        assertEquals(
                "Incorrect transaction date on tx vertex",
                "2015-03-13",
